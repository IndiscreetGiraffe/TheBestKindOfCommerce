...
router.delete('/:id', (req, res) => {
  ...
  .then(tagData => {
    if (!tagData) {
      res.status(404).json({ message: "No tag found, so sorry. Try again"});
      return;
    }
    res.json({ message: "The tag has been deleted, well done!"});
  })
  .catch(err => {
    res.status(500).json({ message: "There has been a error"})
  })
});
